...
DROP TABLE IF EXISTS :loadschema.ptab_f_interactions_call_curr_old;


CREATE TABLE IF NOT EXISTS :loadschema.ptab_f_interactions_call_curr_new
(
  ...
  -- Additional Fields for IVA
    key_message_id bigint ENCODE ZSTD,
	par_source_unique_id varchar(30) ENCODE ZSTD
...
--REM Add Order By   
   INSERT INTO :loadschema.ptab_f_interactions_call_curr_new
(
    interaction_id,
       is_parent_call,
       detail_priority,
...
SELECT 
  ...
  then prh_hl8_product_name 
   ...
   :batchid,
   emp.emp_initials,
   f_interactions_call.key_message_id,
   f_interactions_call.par_source_unique_id
    	   
   

FROM
 :sourcefacts.f_interactions_call f_interactions_call
	LEFT JOIN :sourcedims.d_address add on (f_interactions_call.address_id = add.address_id)
	LEFT JOIN :sourcedims.d_employee emp on (f_interactions_call.employee_id = emp.employee_id)
	INNER JOIN :sourcedims.d_position pos ON (f_interactions_call.position_id = pos.position_id)
	  LEFT JOIN :loadschema.ptab_d_product_full prod on prod.product_id = f_interactions_call.product_id
  LEFT JOIN :loadschema.ptab_d_day_full day on day.day_id = f_interactions_call.day_id
-- Left Joining the LOV tables in case a term definition is missing	
	LEFT JOIN :sourcedims.d_lov call_type_lov ON (f_interactions_call.call_type_lid = call_type_lov.lov_id and call_type_lov.type = 'CALL_TYPE_LID')
	LEFT JOIN :sourcedims.d_lov interaction_sub_type_lov ON (f_interactions_call.interaction_sub_type_lid = interaction_sub_type_lov.lov_id and interaction_sub_type_lov.type ='INTERACTION_SUB_TYPE')
	LEFT JOIN :sourcedims.d_lov interaction_type_lov ON (f_interactions_call.interaction_type_lid = interaction_type_lov.lov_id and interaction_type_lov.type ='INTERACTION_TYPE')
	LEFT JOIN :sourcedims.d_lov detail_type_lov ON (f_interactions_call.detail_type_lid = detail_type_lov.lov_id and detail_type_lov.type = 'DETAIL_TYPE_LID')
	LEFT JOIN :sourcedims.d_lov delivery_status_lov ON (f_interactions_call.delivery_status_lid = delivery_status_lov.lov_id and delivery_status_lov.type = 'DELIVERY_STATUS_LID')
	LEFT JOIN :sourcedims.d_lov child_type_lov ON (f_interactions_call.child_type_lid = child_type_lov.lov_id and child_type_lov.type = 'CHILD_TYPE_LID')
	LEFT JOIN :sourcedims.d_lov product_type_lov ON (f_interactions_call.product_type_lid = product_type_lov.lov_id and product_type_lov.type ='PRODUCT_TYPE')
	LEFT JOIN :sourcedims.d_lov discussion_topics_lov ON (f_interactions_call.discussion_topics_lid = discussion_topics_lov.lov_id and discussion_topics_lov.type ='DISCUSSION_TOPICS_LID')
	LEFT JOIN :sourcedims.d_lov product_strategy_lov ON (f_interactions_call.product_strategy_lid = product_strategy_lov.lov_id and product_strategy_lov.type = 'PRODUCT_STRATEGY_LID')
	LEFT JOIN :sourcedims.d_lov clinical_research_lov ON (f_interactions_call.clinical_research_lid = clinical_research_lov.lov_id and clinical_research_lov.type = 'CLINICAL_RESEARCH_LID')
	LEFT JOIN :sourcedims.d_lov discussion_class_lov ON (f_interactions_call.discussion_class_lid = discussion_class_lov.lov_id and discussion_class_lov.type = 'DISCUSSION_CLASS_LID')
	LEFT JOIN :sourcedims.d_lov device_trained_lov ON (f_interactions_call.device_trained_lid = device_trained_lov.lov_id and device_trained_lov.type = 'DEVICE_TRAINED_LID')
	LEFT JOIN :sourcedims.d_lov call_focus_lov ON (f_interactions_call.call_focus_lid = call_focus_lov.lov_id and call_focus_lov.type = 'CALL_FOCUS_LID')
	LEFT JOIN :sourcedims.d_lov category_lov ON (f_interactions_call.category_lid = category_lov.lov_id and category_lov.type = 'CATEGORY')
	LEFT JOIN :sourcedims.d_lov reaction_lov ON (f_interactions_call.reaction_lid = reaction_lov.lov_id and reaction_lov.type = 'REACTION_LID')
	LEFT JOIN :sourcedims.d_lov activity_type_lov ON (f_interactions_call.activity_type_lid = activity_type_lov.lov_id and activity_type_lov.type = 'ACTIVITY_TYPE_LID')
	LEFT JOIN :sourcedims.d_lov call_status_lov ON (f_interactions_call.call_status_lid = call_status_lov.lov_id and call_status_lov.type = 'CALL_STATUS_LID')
    LEFT JOIN :sourcedims.d_lov attendee_type_lov ON (f_interactions_call.attendee_type_lid = attendee_type_lov.lov_id and attendee_type_lov.type = 'ATTENDEE_TYPE_LID')
	
WHERE f_interactions_call.isdeleted<>'Y' or f_interactions_call.isdeleted is null
);

VACUUM FULL  :loadschema.ptab_f_interactions_call_curr_new to 99 PERCENT;



GRANT TRIGGER, RULE, SELECT, DELETE, UPDATE, REFERENCES, INSERT ON :loadschema.ptab_f_interactions_call_curr_new TO :etluser;
GRANT SELECT ON :loadschema.ptab_f_interactions_call_curr_new TO :readonlyusers;
GRANT SELECT ON :loadschema.ptab_f_interactions_call_curr_new TO :readonlygroups;
...
ALTER TABLE  :loadschema.ptab_f_interactions_call_curr RENAME TO  ptab_f_interactions_call_curr_old;

ALTER TABLE  :loadschema.ptab_f_interactions_call_curr_new RENAME TO  ptab_f_interactions_call_curr;



--Updating latest run stats to apollo control table
UPDATE rpt_apollo.apollo_control_table
set status = 'Successfully Completed', batch_id = :batchid, last_loaded_date = getdate() ,total_records_loaded =  (SELECT count(*) from :loadschema.ptab_f_interactions_call_curr)
WHERE sql_file_name = :'sqlfilename';
