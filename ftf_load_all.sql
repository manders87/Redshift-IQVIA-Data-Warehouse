...
(SELECT ftf.h_ftf_benefit_design_id,
	   ...
	   where ftf.snapshot_date not in (select distinct snapshot_date from rpt_apollo.ptab_fh_ftf_benefit_design_summary)
...
FROM
 ...
 inner join rpt_apollo.v_tab_Time_Period_Attributes time on time.day_id = trx.day_id
  where time."day" > (select dateadd(week,-20, min(snapshot_date)) from rpt_apollo.ptab_fh_ftf_benefit_design_temp)
  and time."day" <= (select max(snapshot_date) from rpt_apollo.ptab_fh_ftf_benefit_design_temp)
...
CREATE TABLE IF NOT EXISTS rpt_apollo.ptab_fh_ftf_benefit_design_summary_new
(
...
where ftf.ftf_therapeutic_class = ftf_distinct.ftf_therapeutic_class
